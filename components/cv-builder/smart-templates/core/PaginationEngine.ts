...
import { CVSection, HeightEstimator, PageContent, TemplateConfig } from './types';
...
export class SmartPaginationEngine {
  ...
  public paginateContent(cvData: any): PageContent[] {
    ...
    const { fullWidthSections, leftColumnSections, rightColumnSections } =
      this.organizeSections(cvData);

    if (this.config.hasColumns) {
      return this.paginateColumnLayout(
        fullWidthSections,
        leftColumnSections,
        rightColumnSections,
        cvData
      ...
      return this.paginateSingleColumn(
        [...fullWidthSections, ...leftColumnSections, ...rightColumnSections],
        cvData
      );
    }
  }
  ...
  private paginateColumnLayout(
    ...
    const totalLeftHeight = leftColumnSections.reduce(
      (sum, section) => sum + this.heightEstimator.getEstimatedHeight(section, cvData),
      0
    );
    const totalRightHeight = rightColumnSections.reduce(
      (sum, section) => sum + this.heightEstimator.getEstimatedHeight(section, cvData),
      0
    );

    console.log(
      `Column heights - Left: ${totalLeftHeight}px, Right: ${totalRightHeight}px, Available: ${availableContentHeight}px`
    );
    ...
    while (leftRemaining.length > 0 || rightRemaining.length > 0) {
      ...
      const { leftRemaining: newLeftRemaining, rightRemaining: newRightRemaining } =
        this.fillColumnsOnPage(overflowPage, leftRemaining, rightRemaining, cvData);
      ...
      if (
        newLeftRemaining.length === leftRemaining.length &&
        newRightRemaining.length === rightRemaining.length
      ) {
  ...
  private fillColumnsOnPage(
    ...
    let leftIndex = 0;
    let rightIndex = 0;
    // Account for internal paddings in the two-column renderer:
    // - First page has extra top padding below header (~50px) + bottom padding (~30px)
    // - Overflow pages typically have a bit less top padding (~20px) + bottom (~30px)
    const isFirstPage = page.sections?.some((s) => s.type === 'header') ?? false;
    const bufferTop = isFirstPage ? 50 : 20;
    const bufferBottom = 30;
    const buffer = bufferTop + bufferBottom;
    // Global safety slack to absorb small estimator errors and borders
    const safetySlack = 16;
    ...
    while (leftIndex < leftSections.length || rightIndex < rightSections.length) {
      ...
      if (leftIndex < leftSections.length) {
        ...
        leftCanAdd = leftHeight + nextHeight <= page.remainingHeight - buffer - safetySlack;
      }

      // Check right column
      if (rightIndex < rightSections.length) {
        const nextHeight = this.heightEstimator.getEstimatedHeight(
          rightSections[rightIndex],
          cvData
        );
        rightCanAdd = rightHeight + nextHeight <= page.remainingHeight - buffer - safetySlack;
      }
    ...
    for (const section of sections) {
      ...
      if (height <= currentPage.remainingHeight - 30 || currentPage.sections.length === 0) {
        ...
        currentPage = {
          sections: [section],
          remainingHeight:
            this.config.pageHeight - this.config.marginTop - this.config.marginBottom - height,
        };
      }
    }
